...
import { DropdownMenu, DropdownItem, Overlay, Field, Icon, Button, Toast, Cell, CellGroup, Radio, RadioGroup, Notify, Empty } from 'vant'

createApp(App).use(store).use(router).use(i18n)
  .use(CalculateMining)
  .use(DropdownMenu).use(DropdownItem).use(Overlay).use(Field).use(Icon).use(Button)
  .use(Toast).use(Cell).use(CellGroup).use(Radio).use(RadioGroup).use(Notify).use(Empty)
  .mount('#app')
